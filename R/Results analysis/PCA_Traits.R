...
morpho_pca <- prcomp(Traits_table_STOC %>% dplyr::select(c(LengthU_MEAN, TailU_MEAN, BillU_MEAN, TarsusU_MEAN, WeightU_MEAN)), scale = T)
summary(morpho_pca)
...
Traits_for_pca <- Traits_table_STOC %>% 
  dplyr::select(c(code_sp,PC1_morpho, PC2_morpho, Clutch_MEAN, Sedentary,
...
global_pca <- PCAmix( X.quanti = as.data.frame(Traits_for_pca %>% dplyr::select(c(PC1_morpho, Clutch_MEAN, Nb_hab, Diet_gen, range_T, Age.of.first.breeding))),
                      X.quali = as.data.frame(Traits_for_pca %>% dplyr::select(c(Sedentary, Nest.type, Trophic.Niche, Lifestyle))), rename.level =  T)
summary(global_pca)
global_pca$eig
