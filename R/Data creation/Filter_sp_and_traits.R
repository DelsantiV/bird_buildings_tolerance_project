...
Dispersion_data <- read.csv(Dispersion_file, sep = ";", dec = ",") %>% rename(code_sp2 = id_species)
...
Traits_table_STOC <- STOC_sp_table_cleaned %>% 
  ...
  left_join(Dispersion_data, by = "code_sp2")

Traits_table_STOC <- Traits_table_STOC %>% dplyr::select(c(code_sp, nom, nom_fr, total_obs_nb, Order, 
                                                    ...
                                                    mean_T, min_T, max_T, range_T, Age.of.first.breeding, Nest.type, natal_dispersal_paradis,
                                                    mean, Incubation.period, Trophic.Niche, Lifestyle, Association.outside.the.breeding.season))
...
Traits_table_STOC$Nest.ground <- Traits_table_STOC$Nest.type == "G"

take_str_end <- function(str, nb_char = 2){
  substring(str, nchar(str)-nb_char+1, nchar(str))
}

Traits_table_STOC$Association.outside.the.breeding.season <- take_str_end(Traits_table_STOC$Association.outside.the.breeding.season)
